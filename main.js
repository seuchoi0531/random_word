$(document).ready(function () {
      ...
      if (i == "\t") {
        if (str.includes("{") && str.includes("^") && str.includes("}")) {
          ...
          Array.from(str).forEach((e) => {
            if (e === "{") newstr += "<ruby><rb>";
            else if (e === "^") newstr += "</rb><rt>";
            else if (e === "}") newstr += "</rt></ruby>";
            else newstr += e;
          });
          str = newstr;
        }
  ...
  function makeTable() {
    ...
    for (let i = 0; i < words.length; i += 2) {
      if (i + 1 < words.length) {
        if (i != 0 && i % 20 == 0) result += `<tr class="boldborder"><td>${words[i].word}</td><td>${words[i].meaning}</td><td>${words[i + 1].word}</td><td>${words[i + 1].meaning}</td></tr>`;
        else if ((i / 2) % 2 == 1) result += `<tr class="oddtr"><td>${words[i].word}</td><td>${words[i].meaning}</td><td>${words[i + 1].word}</td><td>${words[i + 1].meaning}</td></tr>`;
        else result += `<tr><td>${words[i].word}</td><td>${words[i].meaning}</td><td>${words[i + 1].word}</td><td>${words[i + 1].meaning}</td></tr>`;
      } else {
        if (i != 0 && i % 20 == 0) result += `<tr class="boldborder"><td>${words[i].word}</td><td>${words[i].meaning}</td><td></td><td></td></tr>`;
        else if ((i / 2) % 2 == 1) result += `<tr class="oddtr"><td>${words[i].word}</td><td>${words[i].meaning}</td><td></td><td></td></tr>`;
        else result += `<tr><td>${words[i].word}</td><td>${words[i].meaning}</td><td></td><td></td></tr>`;
      }
    }
    result += "</table>";
  ...
  function toggleDrag() {
    if (isDragable) body.setAttribute("onselectstart", "return false");
    else body.setAttribute("onselectstart", "return true");
